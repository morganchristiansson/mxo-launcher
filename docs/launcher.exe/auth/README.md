...
- newer direct `0x43f300 / 0x448140` decompilation also sharpens the current staged-payload boundary:
  ...
  - source consequence:
    - `CLTLoginMediator::HandleAuthPacketBytes(...)` should stay only a thin staging/demux bridge from the replacement's staged payload handoff
  ...
  - fidelity note from the `0x448140` pass:
    ...
    - raw `0x0a` is driven by child `+0x94/+0x98` feedback-transform object ownership in the launcher field graph, but the exact live transform semantics are not yet closed tightly enough to replace the known-working Twofish CBC challenge/response path
    - replacement consequence: keep the raw `0x0a` digest / packet-builder staging inline in `HandleInboundAuthMessage`; preserve the recovered child-field ownership notes, but continue using the known-working direct Twofish challenge decrypt / response encrypt path until the `+0x94/+0x98` transform implementation is proven faithful
    ...
    - raw `0x0b` success-side copy-shadow adoption and `+0xb0/+0xc4/+0xd8` rebuild should stay inline in `AuthBootstrap680Child_0x441290::HandleInboundAuthMessage`, because `0x448140` does that work in-place instead of through a standalone launcher helper boundary
...
Canonical object-level notes for that path now live under:
- `../startup_objects/0x4f78b8_AUTHBOOTSTRAP_CHILD_PLUS680.md`

This auth doc now intentionally omits the detailed bootstrap-child layout/lifecycle duplication.
