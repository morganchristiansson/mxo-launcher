...
flattening it into a vector, and now drives the recovered SGI-tree helper subset through the narrow
project-owned `compat/sgi_tree_compat.h` shim instead of directly including libstdc++ internal tree
headers. Balancing, color-bit meaning, and original node-pool recycling are still only partially
reconstructed.
