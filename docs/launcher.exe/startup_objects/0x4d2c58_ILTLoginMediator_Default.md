...
- current source now mirrors that startup handoff by routing resolved wrapper `+0x08` into
  `CLTLoginMediator::Initialize(networkEngineOverride)`, which installs the engine, active mediator
  source, initial state0 helper, auth-side address-list refresh, and owner `+0x680` child
...
- newer `0x40dbb0/0x40dc40/0x40d8d0/0x40e1c0` tightening also makes the launcher-owned list row
  ...
  - `0x40f180` now closes `g_LauncherSelectionListSortMode004d3588` more tightly as the
    replay/sort-mode selector that feeds comparator `0x40cf40`
...
| `+0x40` | returns selection-descriptor object for the arg7-derived selection index, including name + low-24-bit id data; newer Ghidra tightening says the outer object is a `0x14` record-like header with payload at `+0x10`, broadly matching the common descriptor base used by launcher classes such as `0x004b533c`, but it is still **not** the owner-side `+0x40 = 0x41f2e0 = GetSlotRecordByIndex` accessor; replacement source now keeps this wrapper-facing ABI object explicit and owner-owned on `CLTLoginMediator`, not as ABI-shell globals | medium |
| `+0x44` | returns the current character-slot record object used by the later profile/save path; keep this wrapper-facing ABI object split explicit from the owner-side `+0x688[owner+0xcc8]` accessor family; newer Ghidra tightening says the returned outer object shape matches the concrete `0x004b5328` slot-record layout (`+0x10 payload`, `+0x14 heap string`, `+0x18 len`) closely enough that replacement scratch objects should mirror that class rather than an unrelated ad-hoc stub | high |
